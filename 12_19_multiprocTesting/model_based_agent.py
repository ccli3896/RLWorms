...
class WormRunner():
    ...
    def eval_ep(self,fname,task,steps=300):
        ...
        while st < steps:
            if st%self.act_spacing==0:
                action = self.agent.eval_act(obs)
            next_obs, rew, done, info = self.worm.step(action, task, sleep_time=0) 
    ...
    def full_run(self, num_eps, fname, eps_vector=None,eval_len=300):
        ...
        return grab_im(cam,None)

    def eps_greedy_ep(self,epsilon,task):
