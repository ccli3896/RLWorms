'''
DEVO CHANGES:
Fix issue of sampling from variances instead of standard deviations (match bin_utils_devo.py)
'''
...
class FakeWorm(gym.Env):
    ...
    # Note: the only difference between this and the original ('model_env.py') without bins
    # is in get_sample() and its arguments.
    ...
    def __init__(self, distribution_dict, start=None, ep_len=1e6):
        ...
        obs_on_bins: [12] 
        obs_off_bins: ''
        body_on: [12,2] with dimensions [head angle, mu/sig^2]. 
        body_off: ''
        head_on: ''
        head_off: ''
        reward_on: [12,12,2] with dimensions [body angle, head angle, mu/sig^2]
        reward_off: ''
        '''
    ...
    def get_sample(self,dkey,olds):
        ...
        if 'reward' in dkey:
            mu,sig2 = self.dist_dict[dkey][olds[0],olds[1]]
            return np.random.normal(mu,np.sqrt(sig2))
        else:
            if 'on' in dkey:
                binned_ind = self.dist_dict['obs_on_bins'][olds[1]]
            elif 'off' in dkey:
                binned_ind = self.dist_dict['obs_off_bins'][olds[1]]
            else: 
                raise KeyError('Invalid dict key')
            mu,sig2 = self.dist_dict[dkey][binned_ind,:]
            return self.keep_inside(self.myround(np.random.normal(mu,np.sqrt(sig2))))
